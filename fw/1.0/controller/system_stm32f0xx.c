...
static void SetSysClock(void)
{
  ...
  do
  {
    ...
    StartUpCounter++;
  } while((HSI48Status == 0) && (StartUpCounter != HSI_STARTUP_TIMEOUT));

  if ((RCC->CR2 & RCC_CR2_HSI48RDY) != RESET)
  {
    HSI48Status = (uint32_t)0x01;
  }
  else
  {
    HSI48Status = (uint32_t)0x00;
  }

  if (HSI48Status == (uint32_t)0x01)
  {
    ...
    // /* PLL configuration = HSI48 * 6 = 48 MHz */
  ...
  { /* If HSI48 fails to start-up, the application will have wrong clock
