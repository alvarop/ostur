...
eddystone_types = {0x00: "uid", 0x10: "url", 0x20: "tlm", 0x30: "eid"}
...
def process_ostur_packet(packet, rssi):
    global last_index, last_device_id
    ...
    (
        ...
        battery,
        flags,
    ) = struct.unpack("<HIHhhHH", packet)

    if magic != 0x0579:
        return

    if sample_index != last_index or device_id != last_device_id:
...
def save_csv_data(data):
    ...
    for key in data_columns:
        if key == "timestamp":
            line += datetime.datetime.fromtimestamp(data["timestamp"]).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        elif key == "temperature" or key == "humidity":
            line += str(data[key] / 100.0)
        elif key == "battery":
            line += str(data[key]/1000.0)
        else:
            line += str(data[key])
...
parser.add_argument("--baud_rate", default=115200, type=int, help="BLED112 baud rate")

parser.add_argument("--port", required=True, help="BLED112 device to connect to")
